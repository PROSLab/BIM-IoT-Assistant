...
class Neo4jInfluxChain(Chain):
    ...
    def _call(
        ...
        if not context:
            result: Dict[str, Any] = {
                self.output_key: "It wasn't possible to retrieve the desired information, try again please"}
            return result
        sensor_id = context[0]["sensor.hasGuid"]

        retrieve_sensors_data = InfluxDBTool(sensor_id).run(question)
        if retrieve_sensors_data == 'Error':
            result: Dict[str, Any] = {self.output_key: "There was an error while retrieving the data, please try again"}
            return result
        results = []
